...
function tree2story(; start=" ", stop=" ", min_time=0.0, 
    outdir=current_dir_jld, indir=current_dir_trees, 
    ...
    for iii in start:iterstep:stop
        ...
        if skipexisting
            if in(outfiles).("halo_$(halo_story["rootID"])_$(halo_story["I_SUB"][1]).jld")
                if verbose
                    println("Existing file skipped: halo_$(halo_story["rootID"])_$(halo_story["I_SUB"][1]).jld")
                end
                continue
            end
        end 
        ...
        halo_story["sSFR"][1]   = halo_story["SFR"][1] / read_galaxy_prop(g, "SMST", :physical)[5]
        ...
        for i in 2:length(halo_story["I_SUB"])
            if halo_story["SNAP"][i] < halo_story["SNAP"][i-1]   # This corresponds to an earlier snap and therefore the most massive progenitor
                ...
                try
                    ...
                    halo_story["sSFR"][i]   = halo_story["SFR"][i] / read_galaxy_prop(g, "SMST", :physical)[5]
                catch e
        ...
        for i in loop_length:-1:1
            #print("$i ")
            #println("$i     $(fp_snaps[end]) > $(halo_story["SNAP"][i]) > $(fp_snaps[1])")
            if fp_snaps[end] > halo_story["SNAP"][i] ≥ fp_snaps[1]  # only accept entries within snaps with available stellar spins
                #println("   $(halo_story["SNAP"][i]) == $(fp_snaps[fp_index])")
                ...
                # Identify Mergers
                #println("       $(halo_story["MMP"][i])     $(halo_story["SNAP"][i]) == $(halo_story["SNAP"][i-1])")
            ...
            elseif fp_snaps[end] == halo_story["SNAP"][i] # final checkout
                n_mergers       = vcat( n_mergers, merger_count )
                break
            end
        end
        ...
        merger_collection_STARS = Dict(
                "N_MERGERS"     => n_mergers,
                "SNAP"          => halo_story["SNAP"][fp_indices],
                "ID_ISUB"       => Int.(ones(length(fp_indices)) .* halo_story["I_SUB"][1]),
                "ID_Mfelix"     => ones(length(fp_indices)) .* id_mfelix,
                "ID_M2"         => ones(length(fp_indices)) .* id_m2,
                "I_SUB"         => halo_story["I_SUB"][fp_indices],
        ...
        for i in 1:length(fp_indices)
            ...
            merger_collection_STARS["sSFR"]          = vcat( merger_collection_STARS["sSFR"], halo_story["sSFR"][fp_indices[i]] )
        ...
        save(joinpath(outdir, "halo_$(halo_story["rootID"])_$(halo_story["I_SUB"][1]).jld"), 
