...
function plot_group( snapNR; 
    simbox=current_dir_simbox, res=(1600,900), felixID=" ", subID=" ", lastID=" ", 
    property = "pos", phkeys=true, 
    rad=200, ptsize=3000, arsize=4., port=1688, stepsize=1,
    bgcolor=:white
    , indir=current_dir_jld, spin=true, boxfix=false
    )
    ...
    if spin
        ...
        #@show merger_collection["SNAP"]
        #println(maximum([1,findall(x->x.==snapNR, merger_collection["SNAP"])[1]-1]))
        #println(findall(x->x.==snapNR, merger_collection["SNAP"])[1])
        #println(minimum([length(merger_collection["SNAP"]), findall(x->x.==snapNR, merger_collection["SNAP"])[1]+1]))
        sANGMOM             = hcat( 
            @view(merger_collection["j_main"][:,maximum([1,findall(x->x.==snapNR, merger_collection["SNAP"])[1]-1])])
            , @view(merger_collection["j_main"][:,findall(x->x.==snapNR, merger_collection["SNAP"])[1]])
            , @view(merger_collection["j_main"][:,minimum([length(merger_collection["SNAP"]), findall(x->x.==snapNR, merger_collection["SNAP"])[1]+1])])
             )
        #arrows!(ax,
        arrows!(
            @view(POSITIONS[1,:]), 
